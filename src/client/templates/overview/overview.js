    ...
    if (transactions.length) {
        _.forEach(transactions, function (item) {
            if (accountsId.indexOf(item.account) !== -1) {
                ...
                modified.data.push([new Date(item.date).getTime(), item.amount]);
            } else {
        ...
        var res = _.map(chart, function (item) {
            return {
                name: _.result(_.find(accounts, {_id: item.id}), 'name'),
                data: item.data
            }
        });
    }
